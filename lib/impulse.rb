module Impulse
  class Graph
    ...
    def draw(name, time_scale = :day, params = {})
      ...
      cmd = ["rrdtool graph #{params[:relative_output_path]}/#{params[:filename]} -s -#{seconds_for(time_scale)} #{defaults} -t '#{params[:title]}' -v '#{params[:vertical]}'"]
      
      if name.is_a?(Hash)
        c = 0
        name.each do |k,v|
          v = { :color => '#FF0000', :data => :data }.merge(v)
          cmd << "DEF:d#{c}=#{params[:relative_rrd_path]}/#{v[:name]}.rrd:#{v[:data].to_s}:MAX"
    ...
    def create_or_find_rrd(name, datas , params)
      ...
      unless File.exist?(params[:relative_path] + "/" + name + ".rrd")
        ...
        if datas.is_a?(Hash)
          ...
          system "rrdtool create #{params[:relative_path]}/#{name}.rrd --step 300 --start #{Time.now.to_i - 1} #{entries.join(' ')} #{averages}"
        else
          system "rrdtool create #{params[:relative_path]}/#{name}.rrd --step 300 --start #{Time.now.to_i - 1} DS:data:GAUGE:600:0:U #{averages}"
        end
      end
    end
    
    def push_to_rrd(name, data, params)
      if File.exist?(params[:relative_path] + "/" + name + ".rrd")
        puts 'Updating RRD' if @debug
        system "rrdtool update #{params[:relative_path]}/#{name}.rrd #{Time.now.to_i}:#{data}"
      end
    end
